    ...
    const setupModalEvents = () => {
        ...
        const sendMessage = async () => {
            ...
            try {
                ...
                const apiKey = "AIzaSyBZsNPsBwX5NXclSbHAt-B_Fy-BoX5E_Cg
                
                // Create a copy of the history specifically for the API
                let apiHistory = [...chatHistory];

                // The Gemini API requires the conversation to start with a 'user' message.
                // This removes the hardcoded initial greeting from the API payload only.
                if (apiHistory.length > 0 && apiHistory[0].role === 'model') {
                    apiHistory.shift(); 
                }

                const payload = {
                    // Optional: Add instructions so the AI knows who it is!
                    systemInstruction: {
                        parts: [{ text: "You are a friendly customer service assistant for Partify, an e-commerce store selling car parts like bumpers, fenders, and grilles. Keep your answers concise, helpful, and related to car parts." }]
                    },
                    contents: apiHistory
                };
